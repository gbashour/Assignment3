...
public class CherryController : MonoBehaviour
{
    ...
    public void spawnPeach()
    {
        ...
        if (edge == 1) // Left -- pick random y value, will teleport to edge == 3
        {
            ...
            destination = new Vector3(centrePoint.x + (width / 2), centrePoint.y + centrePointDiff, 0.0f);
        }
        if (edge == 2) // Up -- pick random x value, will teleport to edge == 4
        {
            ...
            destination = new Vector3(centrePoint.x + centrePointDiff, centrePoint.y - (height / 2), 0.0f);
        }
        if (edge == 3) // Right
        {
            float xPos = centrePoint.x + (width / 2);
